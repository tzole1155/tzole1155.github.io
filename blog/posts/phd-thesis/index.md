...
Motion Capture, or MoCap, is the process of digitizing human movement. As a technology, it has its origins in the pioneering works of Étienne-Jules Marey and Eadweard Muybridge — two 19th-century scientists who were obsessed with capturing movement on film long before anyone had heard of a computer.
...
The traditional way to do all of this is expensive and cumbersome.  An actor suits up in a tight uniform covered in reflective markers; infrared cameras track those dots, and algorithms reconstruct a 3D human skeleton in real time. It works almost perfectly — but it costs a lot of money, requires a dedicated lab, and may even require manual cleanup when those markers are occluded.
...
The problem is that achieving the accuracy of high-end optical systems without any of that hardware is one of the long-standing open challenges in computer vision. In this thesis, we tried to reach this goal incrementally by solving three core challenges. 
...
**Noise.** Low-cost cameras introduce signal noise, while AI models introduce their own noise. When both are combined, we get a new type of compound noise that is not effectively handled by existing solvers. In particular, existing approaches require knowing the noise distribution in advance or extensive manual tuning — neither of which is practical in the real world.
...
*The three core challenges addressed in the thesis are data bias, compound noise, and markerless MoCap.*

---

## What this thesis does about it

Differentiating from existing approaches, we aim at solving these challenges using **representation learning**. To make it clearer, imagine mapping all valid human poses onto a manifold—a structured space with properties we can exploit for each of the tasks above.

To address **data bias**, we designed a framework that identifies rare “tail” poses in the training data and synthesizes new examples around them by interpolation in the learned latent space.
...
For the **noise** problem, we built a neural solver that automatically assigns a different level of trust to each landmark. For instance, a torso landmark with less noise, as it is mostly visible to the cameras, is fully trusted; while a flickering, partially occluded hand landmark is down-weighted. No prior knowledge of the noise distribution needed, no manual configuration.

![Noise framework](assets/phd-defense/noise_concept.jpg)
*Noise modelling.*

To address the **temporal inconsistency** problem, we introduced **BundleMoCap** — a method that solves for specific keyframes and synthesises intermediate frames by interpolating poses on the manifold. By optimizing over a short window of frames, the solution naturally becomes smooth and physically plausible, without defining explicit smoothness terms.

![BundleMoCap smooth motion sequence](assets/phd-defense/bundlemocap_sequence.png)
*BundleMoCap produces smooth, temporally consistent motion by jointly solving a bundle of frames.*

---

## Demos

To demonstrate the practicality of the proposed methods beyond public benchmarks, we presented them live at **ICCV 2023 in Paris** and **CVMP 2023 in London**. 
